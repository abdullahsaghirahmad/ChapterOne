...
class NLPService {
    ...
    async determinePace(text, pageCount) {
        if (pageCount) {
            if (pageCount < 300)
                return 'Fast';
            if (pageCount < 600)
        ...
        for (const [pace, indicators] of Object.entries(paceIndicators)) {
            scores[pace] = Array.isArray(tokens) ? indicators.filter(indicator => tokens.includes(indicator)).length : 0;
        }
    ...
    async determineBestFor(text, categories, pageCount) {
        const audiences = new Set();
        // Check categories
        if (categories) {
            if (categories.some(cat => cat.toLowerCase().includes('juvenile') ||
                cat.toLowerCase().includes('children'))) {
                audiences.add('Children');
            }
            if (categories.some(cat => cat.toLowerCase().includes('young adult'))) {
        ...
        if (text) {
            const tokens = this.tokenizer.tokenize(text.toLowerCase()) || [];
            if (Array.isArray(tokens) && tokens.some(token => ['child', 'children', 'kid', 'kids', 'young', 'youth'].includes(token))) {
                audiences.add('Children');
            }
            if (Array.isArray(tokens) && tokens.some(token => ['teen', 'teenager', 'adolescent', 'young adult'].includes(token))) {
                audiences.add('Young Adults');
            }
            if (Array.isArray(tokens) && tokens.some(token => ['simple', 'easy', 'basic', 'beginner'].includes(token))) {
                audiences.add('Casual Readers');
            }
            if (Array.isArray(tokens) && tokens.some(token => ['complex', 'challenging', 'advanced', 'sophisticated'].includes(token))) {
                audiences.add('Avid Readers');
            }
        }
