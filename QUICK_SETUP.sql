-- 🚀 Quick Setup for ChapterOne ML Feature Flags
...
CREATE POLICY IF NOT EXISTS "Feature flags are readable by everyone" ON feature_flags
FOR SELECT USING (true);

-- Allow updates to feature flags in development (for the toggle UI)
CREATE POLICY IF NOT EXISTS "Feature flags are updatable in development" ON feature_flags
FOR UPDATE USING (environment = 'development');

-- Allow all operations on ML tables for development
CREATE POLICY IF NOT EXISTS "ML tables accessible for development" ON user_contextual_preferences
FOR ALL USING (true);

CREATE POLICY IF NOT EXISTS "Impressions accessible for development" ON recommendation_impressions
FOR ALL USING (true);

CREATE POLICY IF NOT EXISTS "Actions accessible for development" ON recommendation_actions
FOR ALL USING (true);
